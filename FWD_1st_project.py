...
def get_filters():
    ...
    city=input('chicago or new york or washington')
    while city not in (CITY_DATA.keys()):
        print ('you should chose from chicago or new york city or washington')
        city=input('chicago or new york or washington').lower()
...
#view data    
def view_data(data):
    #ask user if he want to display 5 rows of data frame
    raw_data=input("Would you like to view 5 rows of individual trip data? Enter yes or no?")
    if raw_data.lower() =='yes':
        #make it start from index 0 our first row
        count =0
        while True:
            print(data.iloc[count:count+5])
            count +=5
            #then take different row each time user say yes
            ask_again=input('next 5 raws?')
            if ask_again.lower() !='yes':
                #break  loop when user say no 
                break
